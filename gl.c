...
byte load_tex_compressed(unint t, const char *fname, tex_filter tf) {
  ...
  if (fload(fname, &size, (void **)&data)) {
    ...
    memcpy((void *)&fmt, (const void *)data, sizeof(GLenum));
    offset += sizeof(GLenum);
    if (tf == tf_mipmap)
      memcpy((void *)&mips, (const void *)(data + offset), sizeof(byte));
    offset += sizeof(byte);
    ...
    for (i = 0; i < mips; i++) {
      ...
      memcpy((void *)&w, (const void *)(data + offset), sizeof(unshort));
      offset += sizeof(unshort);
      memcpy((void *)&h, (const void *)(data + offset), sizeof(unshort));
      offset += sizeof(unshort);
      memcpy((void *)&size, (const void *)(data + offset), sizeof(int));
      offset += sizeof(int);

      glCompressedTexImage2D(GL_TEXTURE_2D, i, fmt, w, h, 0, size,
    ...
    if (mips > 1) {
      if (_anis == -1) {
        ...
        if (_anis == 0)
          info("Anisotropic texture filtering not supported\n");
      }
      if (_anis > 0)
